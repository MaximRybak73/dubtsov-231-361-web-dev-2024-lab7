...
function compile(str) {
    let out = [];
    let stack = [];
    for (token of tokenize(str)) { //разбиваем строку на токены, далее создается стек для операторов и 
                                    //массив для выходного выражения
                                    //затем преобр-ние в опн
        if (isNumeric(token)) {
            out.push(token);
...
function evaluate(str) {
    let stack = [];
    let tokens = str.split(' '); //разделить строку на токены

    for (let token of tokens) {
        if (isNumeric(token)) {
            stack.push(parseFloat(token)); // Если число, поместить в стек
        } else if (isOperation(token)) { //если встречается оператор, 
                                         //из стека извлекаются два числа, и выполняется операция
            let b = stack.pop();
            let a = stack.pop();
            switch (token) {
                case '+':
    ...
    //после завершения работы в стеке остается один элемент — результат вычисления
}
...
function clickHandler(event) {
    const screen = document.querySelector('.screen span');// переменная screen будет ссылаться на элемент span, 
                                                          //внутри которого будет отображаться выражение
    const button = event.target; //определить какая кнопка была нажата

    if (button.classList.contains('digit') || button.classList.contains('operation') || button.classList.contains('bracket')) {
        screen.textContent += button.textContent;
    } else if (button.classList.contains('clear')) {
        screen.textContent = ''; //очистить экран
    } else if (button.classList.contains('result')) {
        const expression = screen.textContent;

        // Проверка, что строка не пустая
        if (expression.trim() === '') {
            screen.textContent = 'Ошибка'; // при пустом вводе
            return;
        }

        try {
            const rpn = compile(expression);// в польскую нотацию
            const result = evaluate(rpn); 
            screen.textContent = result; //отобразить на экране
        } catch (error) {
...
window.onload = function () {
    document.querySelector('.buttons').addEventListener('click', clickHandler); //добавление обработчика события нажатия мыши
}
